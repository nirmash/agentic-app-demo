...
${indent}  </div>
${indent}  <div class="form-group-body">
${indent}    <textarea class="form-control input-block" id="${id}" name="${id}" placeholder="${escapeHtml(field.placeholder || '')}" rows="${field.rows || 4}"${req}></textarea>
${indent}  </div>
${indent}</div>`;
...
${indent}</div>`;
    ...
    case 'lookup': {
      ...
      const source = escapeHtml(field.source || '');
      const displayField = escapeHtml(field.displayField || 'name');
      const valueField = escapeHtml(field.valueField || 'sessionId');
      return `${indent}<div class="form-group mb-3">
${indent}  <div class="form-group-header">
${indent}    <label class="FormControl-label" for="${id}">${escapeHtml(field.label)}${field.required ? ' <span class="color-fg-danger">*</span>' : ''}</label>
${indent}  </div>
${indent}  <div class="form-group-body">
${indent}    <select class="form-select input-block" id="${id}" name="${id}"${multi}${req} data-lookup-source="${source}" data-lookup-display="${displayField}" data-lookup-value="${valueField}">
${indent}      <option value="">Loading…</option>
${indent}    </select>
${indent}  </div>
${indent}</div>`;
    }
    ...
    case 'link':
      const linkHref = (field.href || '#') + (field.href && !field.href.includes('?') ? '?new=true' : field.href && field.href.includes('?') ? '&new=true' : '');
      return `${indent}<div class="mb-3">
${indent}  <a class="Link" href="${escapeHtml(linkHref)}">${escapeHtml(field.label || 'Link')}</a>
${indent}</div>`;
  ...
  <div class="form-container">
    ...
    <nav aria-label="Breadcrumb" class="mb-3">
      <ol class="breadcrumb" style="list-style:none; display:flex; gap:0.25rem; padding:0; margin:0; font-size:14px;">
        ...
        <li class="breadcrumb-item breadcrumb-item-selected" aria-current="page">${escapeHtml(spec.title)}</li>
      </ol>
    </nav>
...
${sectionsHtml}
    ...
    <div id="record-nav" style="display:none; margin-top:1rem; padding:0.75rem; background:var(--bgColor-muted,#161b22); border:1px solid var(--borderColor-default,#30363d); border-radius:6px;">
      <div style="display:flex; align-items:center; gap:0.5rem; flex-wrap:wrap;">
        <a href="/" class="btn btn-sm" title="Back to Home">🏠 Home</a>
  ...
  <script>
    ...
    const isNewRecord = urlParams.get('new') === 'true';
    if (loadId) SESSION_ID = loadId;

    // Populate lookup dropdowns from source form records
    document.querySelectorAll('select[data-lookup-source]').forEach(async function(sel) {
      const source = sel.dataset.lookupSource;
      const displayField = sel.dataset.lookupDisplay || 'name';
      const valueField = sel.dataset.lookupValue || 'sessionId';
      try {
        const res = await fetch(API_BASE + '/api/records/' + source);
        const result = await res.json();
        sel.innerHTML = '<option value="">Select…</option>';
        if (result.ok && result.records) {
          result.records.forEach(function(rec) {
            const opt = document.createElement('option');
            opt.value = valueField === 'sessionId' ? rec.sessionId : (rec.data[valueField] || rec.sessionId);
            opt.textContent = rec.data[displayField] || rec.sessionId;
            sel.appendChild(opt);
          });
        }
      } catch(e) {
    ...
    function populateForm(data) {
      ...
      for (const [key, value] of Object.entries(data)) {
        ...
        if (Array.isArray(value)) {
          ...
          // Multi-select lookup: set selected options
          const selEl = form.querySelector('select[name="' + key + '"][multiple]');
          if (selEl) {
            const vals = value.map(String);
            Array.from(selEl.options).forEach(opt => { opt.selected = vals.includes(opt.value); });
            continue;
          }
          continue;
        }
    ...
    async function loadRecordById(id) {
      try {
        const res = await fetch(API_BASE + '/api/load?formName=' + FORM_NAME + '&id=' + id);
        if (!res.ok) return;
        const result = await res.json();
    ...
    async function navigateRecord(direction) {
      ...
      if (newIdx < 0 || newIdx >= recordsList.length) return;
      currentRecordIdx = newIdx;
      await loadRecordById(recordsList[newIdx].sessionId);
      updateRecordNav();
    }
    ...
    async function fetchRecordsList() {
      try {
        ...
        if (!data.ok || !data.records || data.records.length === 0) {
          if (isNewRecord) recordNav.style.display = '';
          return;
        }
        recordsList = data.records;
        recordNav.style.display = '';
        // If a specific record was requested via ?id=, find its index
        if (loadId) {
          const idx = recordsList.findIndex(r => r.sessionId === loadId);
          if (idx >= 0) currentRecordIdx = idx;
        }
        // Auto-load first record if none is selected (unless ?new=true)
        if (currentRecordIdx < 0 && recordsList.length > 0 && !isNewRecord) {
          currentRecordIdx = 0;
          await loadRecordById(recordsList[0].sessionId);
        }
        updateRecordNav();
      } catch {}
    }
    ...
    // Load records list and auto-display first record on page load
    fetchRecordsList();
    ...
    async function saveCurrentRecord() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          // Refresh records list after save
          const savedId = SESSION_ID;
          currentRecordIdx = -1;
          await fetchRecordsList();
          const idx = recordsList.findIndex(r => r.sessionId === savedId);
          if (idx >= 0) currentRecordIdx = idx;
          updateRecordNav();
        } else {
    ...
    <div class="overflow-auto" id="table-wrap" style="display:none">
      <table class="list-table">
        <thead>
          <tr>
${thHtml}
            <th class="p-2" style="width:70px">Edit</th>
          </tr>
        </thead>
...
${cards}
    ...
    <div class="Subhead mt-4 mb-3">
      <h2 class="Subhead-heading">🛠️ Tools</h2>
    </div>
    <div class="d-flex" style="gap: 16px;">
      <a href="/db/" class="Box p-3 d-flex flex-items-center flex-auto" style="border-radius:8px; text-decoration:none; color:inherit;">
        <span class="mr-2" style="font-size:1.4em;">🗄️</span>
        <div>
          <strong class="f4">Database Explorer</strong>
          <div class="color-fg-muted" style="font-size:13px;">View PostgreSQL tables and run SQL queries</div>
        </div>
      </a>
      <a href="/cli/" class="Box p-3 d-flex flex-items-center flex-auto" style="border-radius:8px; text-decoration:none; color:inherit;">
        <span class="mr-2" style="font-size:1.4em;">⌨️</span>
        <div>
          <strong class="f4">CLI Terminal</strong>
          <div class="color-fg-muted" style="font-size:13px;">Run adcgen commands to generate and manage forms</div>
        </div>
      </a>
    </div>
  </div>
